...
const startIndex = content.indexOf('export const Projects: React.FC = () => {');
if (startIndex !== -1) {
  content = content.substring(0, startIndex);
  const newComponent = `export const Projects: React.FC = () => {
...
`;
  fs.writeFileSync(file, content + newComponent, 'utf8');
}
